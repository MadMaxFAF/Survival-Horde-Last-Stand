...
CreateWaves = function()
    WaveTable = import('/maps/Survival_horde_last_stand_prep.v0001/src/WaveTable.lua').getWaveTable(
        EnableLane1,
        EnableLane2,
        EnableLane3,
        EnableLane4,
...
CreateWaveTableOrders = function()
    ...
    WaveOrders = import('/maps/Survival_horde_last_stand_prep.v0001/src/WaveOrders.lua').getWaveTable(
        WaveDelay,
        ShuffleList,
...
--    local survival = import('/maps/Survival_horde_last_stand_prep.v0001/src/Survival.lua').newInstance(
--        ScenarioInfo,
--        CreateWaves,
...
ForkThread(function()
    local textPrinter = import('/maps/Survival_horde_last_stand_prep.v0001/src/lib/TextPrinter.lua').newInstance()
    ...
    textPrinter.print(string.rep( " ", 12 ) .. "Welcome to Survival DDay Horde", headerOptions)
    textPrinter.printBlankLine(textOptions)
    textPrinter.print(string.rep( " ", 20 ) .. "Version 1", textOptions)
    textPrinter.printBlankLine(textOptions)
    textPrinter.printBlankLine(textOptions)
    textPrinter.print(string.rep( " ", 20 ) .. "Remember to build Anti-Air!", textOptions)
end)

local function newAirwingSpawner()
    local mapSizeX, mapSizeY = GetMapSize()
    return import('/maps/Survival_horde_last_stand_prep.v0001/src/lib/AirwingSpawner.lua').newInstance(mapSizeX, mapSizeY, "ARMY_SURVIVAL_ENEMY")
end

ForkThread(function()
    local textPrinter = import('/maps/Survival_horde_last_stand_prep.v0001/src/lib/TextPrinter.lua').newInstance()
    if ScenarioInfo.Options.opt_HordeAirWaves == 1 then
        local airwings = import('/maps/Survival_horde_last_stand_prep.v0001/src/Airwings.lua').newInstance(
            newAirwingSpawner(),
            textPrinter,
            TotalGameTime
        )
...
local function setupAutoReclaim()
    if ScenarioInfo.Options.opt_HordeAutoReclaim > 0 then
        ForkThread(
            import('/maps/Survival_horde_last_stand_prep.v0001/src/lib/AutoReclaim.lua').AutoResourceThread,
